...
for ll = 1:k
    if update(ll)
        L  = l(ll);
        [CC, SS] = nneg_lasso(y_sub, ref, L, augnum, iter);
        C(:,:,ll) = CC(:,:,ll);
        ref_new(:,ll) = SS(:,ll);
    end
end
...
%% Step 5: Display concentration maps and basis spectra
...
figure;
for ii = 1:k
    plot(Raman_shift, ref_new(:,ii) + (ii-1), 'LineWidth', 1); hold on
end
hold off
set(gca,'XMinorTick','on','YMinorTick','on');
xlabel('Raman shift (cm^{-1})'); ylabel('Int (a.u.)')
